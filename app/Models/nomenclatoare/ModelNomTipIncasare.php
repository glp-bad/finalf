...
class ModelNomTipIncasare extends MyModel {
    ...
    public function selectForSimpleDropDown(){
        $rezult = DB::select(
            "select id, cIncas as text, 'false' as selected from t_tip_incasare order by id;"
        );
    ...
    public function selectEntity($id){
        $rezult = DB::select(
            " select id, cIncas from t_tip_incasare where id = ?;",[$id]
        );
